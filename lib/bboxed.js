...
Parser.prototype.parseGroup = function(endTag) {
	...
	while (token = this.peek()) {
		...
		if (token.type == 'text') {
			console.log('parsed text:', token.text);
			result += token.text;
			continue;
		}
		...
		if (token.tag == endTag && token.closing) {
			console.log('reached closing tag for:', token.tag);
			return result;
		}
		...
		if (token.closing || !tag) {
			console.log('parsing stray closing as text');
			result += token.text;
			continue;
		}
		...
		if (tag.hasCloseTag) {
			console.log('opening group tag:', token.tag);
