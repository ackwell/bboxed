...
Lexer.tokenise = function(input) {
	...
	while (input) {
		if (match = tokens.tag.exec(input)) {
			...
			if ((match[2] === '*' || match[2] === '#') && match[1] !== '/' && insideList === false) {
				insideList = true; //only jump ahead and replace if we aren't currently parsing in a list
				var newLine = 0;
				do { // find next /n[*] or /n[#]
					if (input.indexOf('\n',newLine + 1) >= 0) { //we are at next list item
						newLine = input.indexOf('\n',newLine + 1);
						input = input.substr(0, newLine-1) + '[/' + match[2] + ']' + input.substr(newLine+1); //insert closing [/*]
						newLine = newLine + 3;
					}
					else { //there are no newlines left in entire parse string
						input = input + '[/' + match[2] + ']'; //insert closing [/*]
						newLine = input.length; //move newLine to end of string
					}
				} while ((input.substr(newLine, 3) === match[0] || input.substr(newLine, 4) === match[0]) && newLine >= 0)

				if (match[2] === '*') { //wrap the list in a ulist bbcode
					input = '[ulist]' + input.substr(0, newLine) + '[/ulist]' + input.substr(newLine);
					match = tokens.tag.exec(input);
				}
				else { //wrap the list in an olist bbcode
					input = '[olist]' + input.substr(0, newLine) + '[/olist]' + input.substr(newLine);
					match = tokens.tag.exec(input);
				}
			}
			if (match[0] === '[/ulist]' || match[0] === '[/olist]') //the parser has reached end of the list block
				{
					insideList = false;
				}
