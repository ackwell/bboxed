...
Parser.prototype.parseGroup = function(endTag, treatAllAsText) {
	...
	while (token = this.peek()) {
		...
		// If it's a stray closing tag, a tag we don't understand, or
		// the function has been told to parse all as text, treat as text
		var tag = this.tags[token.tag];
		if (token.closing || !tag || treatAllAsText) {
			result += token.text;
			continue;
		}
...
Parser.parse = function(tokens, tags) {
...
function bboxed(input, callback) {
	var tokens = Lexer.tokenise(input);

	if (callback) {
		var done = function() {
			var out, error;
			try { out = Parser.parse(tokens, bboxed.tags); }
			catch (e) { error = e; }
	...
	return Parser.parse(tokens, bboxed.tags);
}
